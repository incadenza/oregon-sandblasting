...
type Props = {
  data?: {
    ...
    blastToFinishCtaLink?: any
    blastToFinishBackgroundImage?: any
  } | null
}

export default function FromBlastToFinish({data}: Props) {
  const heading = data?.blastToFinishHeading || 'From Blast to Finish'
  const body =
    data?.blastToFinishBody ||
  ...
  const ctaLabel = data?.blastToFinishCtaLabel || 'Explore Our Services'
  const ctaHref = linkResolver(data?.blastToFinishCtaLink) || '/services'
  const bgUrl =
    urlForImage(data?.blastToFinishBackgroundImage)?.width(1800).url() ||
    '/figma-assets/f16181ac8df6f6ba9445c9839ec6c317f02fbffa.png'
  const bgAlt =
    data?.blastToFinishBackgroundImage?.alt ||
      ...
      <div className="container">
        <div className="relative overflow-hidden bg-[#3f3724]">
          <div className="absolute inset-0">
            <img alt={bgAlt} src={bgUrl} className="h-full w-full object-cover object-left" />
          </div>
